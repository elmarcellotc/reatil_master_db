...
Once in docker container termina, to access to mysql terminal use:
```bash
mysql -u root -p"$MYSQL_ROOT_PASSWORD"
```

If you want to count how many rows and columns are in each tables in RetailMasterDB, call this script once you are using RetailMasterDB in MySQL:
```bash
SOURCE init-db/count_rows_by_table.sql;
```
